...
def eqs_of(disp):
    clean=disp.replace("\(","|").replace("\)","|")
    parts=[p for p in clean.split("|") if "=" in p]
    return parts
...
for tier in ["bronze","silver","gold"]:
    ...
    for i,p in enumerate(pb[tier]):
        ...
        eqs=eqs_of(disp)
        if len(eqs)<2:
            errors.append(("PARSE",f"{tier}[{i}]",disp)); continue
        x,y=solve_pair(eqs)
...
# teach walks + opener solve check
for tier,walk in live["guided"]["teach"].items():
    eqs=eqs_of(walk["display"])
    x,y=solve_pair(eqs)
    if abs(x-round(x))>1e-9 or abs(y-round(y))>1e-9:
        errors.append(("TEACH_MESSY",tier,f"{walk['display']}->({x},{y})"))
    print(f"teach.{tier}: {walk['display']} solves to ({x:g},{y:g})")
...
if pre_pd:
    for fld in ["related_videos","topic_links","worked_examples","method_card"]:
        ...
        if a!=b: errors.append(("PRESERVE",fld,f"PRE={a[:100]} || LIVE={b[:100]}"))
    print("\npre fields:", sorted(pre_pd.keys()))
    print("live fields:", sorted(live.keys()))
    print("pre has guided?", "guided" in pre_pd, "| pre has tier_guides?", "tier_guides" in pre_pd)

print("\n=== FINDINGS ===")
for e in errors: print(e)
print("total non-preserve-info findings:",len(errors))
