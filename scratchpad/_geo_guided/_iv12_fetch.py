...
json.dump(pd, open(os.path.join(d,"_iv12_live.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=1)
print("top keys:", list(pd.keys()))
pb = pd.get("problem_bank") or {}
print({t: len(pb.get(t) or []) for t in ("bronze","silver","gold")})
print("pb keys:", [k for k in pb.keys()])
print("guided keys:", list((pd.get("guided") or {}).keys()))
