...
(function () {
  ...
  var CSS = '.svtimer{position:relative;display:inline-flex;align-items:center;gap:6px;border:0;background:none;padding:2px 4px;cursor:pointer;font:600 .82rem Inter,system-ui,sans-serif;color:#4a4239;flex:none}'
    + '.svtimer .dial{width:26px;height:26px;display:block}'
    + '.svtimer .dial .ring{fill:#fffdf8;stroke:#3a2615;stroke-width:1.6}'
    + '.svtimer .dial .arc{fill:none;stroke:#c06325;stroke-width:3.2;stroke-linecap:round;transform:rotate(-90deg);transform-origin:50% 50%;transition:stroke-dashoffset .9s linear}'
    + '.svtimer .dial .tick{stroke:#3a2615;stroke-width:1.2}'
  ...
  function mount(where) {
    ...
    b.innerHTML = '<svg class="dial" viewBox="0 0 26 26" aria-hidden="true"><circle class="ring" cx="13" cy="13" r="12"/>'
      + [0, 90, 180, 270].map(function (a) { return '<line class="tick" x1="13" y1="2.5" x2="13" y2="4.5" transform="rotate(' + a + ' 13 13)"/>'; }).join('')
      + '<circle class="arc" cx="13" cy="13" r="' + R + '" stroke-dasharray="' + C.toFixed(2) + '" stroke-dashoffset="' + C.toFixed(2) + '"/></svg><span class="left"></span>';
