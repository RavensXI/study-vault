...
function initPracticeQuestions() {
  ...
  function formatAiResponse(text) {
    ...
    h = h.replace(/\*\*([^*\n]+?)\*\*/g, '<strong>$1</strong>');
    // Single-asterisk italics. Without this the marker's emphasis leaked into the
    // feedback as literal asterisks — *at the time*, *why* — which a student reads
    // as typing errors (seen on a projector-sized screenshot, 21 Sep 2026).
    h = h.replace(/(^|[^*])\*(?!\s)([^*\n]+?)\*(?!\*)/g, '$1<em>$2</em>');
    h = h.replace(/\*\*/g, '');
