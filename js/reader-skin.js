/* Reader skin — the shipped half of design-lab/skin-switcher.js.
 *
 * The switcher itself never ran in production: it opens with a hostname check
 * that returns unless you are on localhost or a *.vercel.app preview, and every
 * template loaded it as "DESIGN-LAB only; remove before shipping". So the
 * redesigned lesson markup went live with the skin switched off and collapsed.
 *
 * Differences from the switcher, all deliberate:
 *   - no hostname gate; this is meant to run on the live domain
 *   - no dev toolbar (it was already hidden — it covered the header buttons)
 *   - defaults to READER, not desk. desk-world needs the Bindery lockup and
 *     painted hero assets, which are gitignored pending trademark clearance
 *     and excluded from the deploy, so it cannot ship.
 *   - no ?csprobe / ?dark QA hooks
 * Everything positional — tidy(), watchOldOnboarding(), applySubjectAccent() —
 * is kept as-is: the redesigned templates rely on it to unwind the old layout.
 */
(function () {
  ...
  function init() {
    ...
    // Two moments matter: the progress box (the count) and the tutor dock (the tool tiles and
    // the panel wait for it). tidy() runs once at each, then the observer stops: no loop.
    var seenProgress = false, seenTutor = false;
    var early = new MutationObserver(function () {
      var p = !seenProgress && document.querySelector('.sidebar-progress-section');
      var t = !seenTutor && document.querySelector('.tutor-dock');
      if (p) seenProgress = true;
      if (t) seenTutor = true;
      if (seenProgress && seenTutor) early.disconnect();
      if (p || t) tidy();
    });
