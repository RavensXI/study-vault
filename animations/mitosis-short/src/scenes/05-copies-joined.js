// STUB
// Placeholder for shot 05 'copies-joined' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'copies-joined',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('copies-joined');
    L.paper(ctx);
    ...
    L.text(ctx, info.shot.title || 'copies-joined', 540, 1420, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'copies-joined', 540, 1480, { size: 30, align: 'center', color: P.inkSoft });
