...
## Active TODO
- **Lesson tutorial LIVE** (26 Apr 2026) — First-time onboarding tour for article lessons. Sequential coachmark steps with darkened backdrop (4 fixed panels around target — works through `overflow:hidden` ancestors and on fixed-position elements unlike box-shadow tricks). Steps auto-resolve at runtime so missing features (no glossary terms, no podcast, no cinematic video) just drop their step. Steps are sorted by `getBoundingClientRect().top + scrollY` so the tour walks top-to-bottom; bug FAB is `pinLast: true` since fixed-position elements have viewport-relative rects. On finish: smooth scroll to top + dark toast "You're all set. Happy revising!" Skipping the tour suppresses the toast. Dismiss is sticky via `localStorage.sv-lesson-tutorial-done`. **Replay button** lives under the lesson checklist — small icon-only circle inside the gutter on ≥1400px, full pill in the right sidebar on narrower viewports. Code: `js/lesson-loader.js` (`maybeStartLessonTutorial`, `addSidebarTourButton`). CSS: `.sv-tutorial-tooltip`, `.sv-tutorial-backdrop`, `.gutter-tour-btn` in global stylesheet. **Critical gotcha:** spotlight class must NOT set `position: relative` — overrides fixed-position targets and drops them into normal flow.
- **Bug reporter LIVE** (26 Apr 2026) — Floating dark FAB bottom-right on every public page (`/js/bug-reporter.js`, auto-skips on `/admin/*` and `/teacher/*`). Modal: description (required) + email (optional) + "include screenshot" toggle (on by default). Lazy-loads html2canvas from CDN, captures viewport, downscales to 1600px max edge JPEG q78 (~200–400KB), POSTs to `/api/bug-report` which uploads to R2 (`studyvault-images/bug-reports/{date}/{hash}.jpg`) and inserts into `bug_reports` table. Per-IP rate limit 10/hour. Admin triage at `/admin/bugs` (linked from every admin nav): card list with click-to-zoom screenshot lightbox, status filter (open/investigating/fixed/wontfix), inline status changes + delete. Schema: `scripts/_create_bug_reports.sql`.
- **Subject request feature LIVE** (26 Apr 2026) — Public form on homepage (collapsible `<details>` panel under subject grid) and inside the free wizard step 1. Fields: subject (req), topic/unit (optional, useful for History where boards offer many topics), exam board (optional dropdown), email (optional). POSTs to `/api/subject-request`, stored in `subject_requests` table (`subject_name, topic, exam_board, email, notes, status, user_agent, created_at`). Per-IP rate limit 5/hour. Admin triage at `/admin/requests`: pending/building/live/rejected statuses, CSV export. Schema: `scripts/_create_subject_requests.sql` (idempotent, ADD COLUMN IF NOT EXISTS for the topic field).
- **About + FAQ pages LIVE** (26 Apr 2026) — `/about` and `/faq` rewrite the AI transparency story head-on. About explains the 4-stage content pipeline (plan → AI draft → assets → review) and names the actual tools (Anthropic Claude for text, Azure TTS, NotebookLM for podcasts, Unsplash for hero photos). FAQ tackles "is this AI-generated" / "will it be wrong" / "will it replace my teacher" / environmental impact / AI marking. Linked from a refreshed legal footer (About · FAQ · Copyright · Privacy) on every public page.
- **Deck-reveal animation LIVE** (26 Apr 2026) — Shimmer placeholders flip into real cards with a stagger after personalisation moments. Reusable helper `window._svDeckReveal(container, options)` in `/js/deck-reveal.js`, loaded by index.html and browse.html. CSS in global stylesheet (`.home-card-shimmer`, `.unit-card-shimmer`, `@keyframes deck-card-reveal` with rotateY 75° → -6° overshoot → 0°). Used in: (1) Unity tier-picker close (homepage), (2) Free wizard completion (sessionStorage `sv-just-personalised` flag triggers it after the post-save reload), (3) Browse page first visit per subject (`localStorage.sv-browse-seen-{slug}` — repeat visits skip to instant reveal so they feel snappy). 110ms stagger, 280ms initial pause. **Critical:** every `.home-card` has `.sv-reveal` baked in (opacity:0) so when deck-reveal class is removed, also add `.sv-visible` to keep the card visible after the animation lands.
- **Lesson notice modal LIVE** (26 Apr 2026) — Generic mechanism for "this is a generic version, your school's is different" alerts. Hidden `<div class="lesson-notice" data-notice-title="..." hidden>...</div>` block at the top of `content_html` is extracted by `lesson-loader.js` on render and shown as a modal that requires "Got it" to dismiss. SessionStorage-keyed per lesson so it doesn't re-fire each navigation. Currently used on 11 free-tier Geography fieldwork lessons (where each school does different fieldwork sites). Documented in `docs/LESSON_TEMPLATE.md` as a first-class component.
- **Subject build verifier LIVE** (26 Apr 2026) — `scripts/_verify_subject_build.py {slug}` is now the canonical pre-ship check. Catches: units missing `image_url`, malformed `quote_ticker_html` (must have `<div class="quote-ticker"><div class="quote-ticker-track">` wrapper — bare spans don't animate), too few revision-technique guide pages, lessons missing description / hero / related_media, related_media coverage gaps (≥1 Podcast, ≥1 Video, ≥1 Movie/TV/Doc, ≥1 Study Tool, ≥6 total per article lesson), `items: null` in any related_media category (caused a crash on Geography Edexcel A — null.forEach in renderer), Gemini diagrams in free-tier content, fieldwork-keyword lessons missing the `lesson-notice` block. YouTube refs verified via oembed (NOT curl HEAD — YT returns 200 for deleted videos). Phase 6 of `docs/PIPELINE.md` requires running it before ship.
- **Geography free-tier related_media refilled** (26 Apr 2026) — 20 article lessons across Edexcel A (15), Edexcel B (4), and OCR (1) had stripped/null-items related_media after the Mumbai DW Doc cleanup. Refilled via 4 parallel curation agents — each lesson now has ≥6 oembed-verified items spanning Podcasts / Videos / Movies-TV-Docs / Study Tools. Plus 4 Edexcel A Paper 3 lessons got a Movies/TV/Docs top-up via a follow-up agent. All 3 boards now PASS the verifier. Defensive renderer fix in `js/lesson-loader.js` so `items: null` no longer crashes the loader (`Array.isArray()` coercion + skip empty categories).
- **Homepage count fix for school students** (26 Apr 2026) — Unity homepage was showing doubled counts (History 8 units / 96 lessons instead of 4 / 60; English Lit 33 / 256; Science 18 / 170) because `js/home-counts.js` summed both the school's bespoke subject AND the generic same-slug row. Fixed by filtering to bespoke-only when both exist for school-logged-in viewers. Free-tier users only see generic anyway, unaffected.
- **Homepage MutationObserver runaway fix** (26 Apr 2026) — Setting `textContent` always triggers a DOM mutation even when the value is unchanged, so `home-counts.js`'s observer-on-grid was infinite-looping after the Unity tier picker re-rendered the grid (page-unresponsive dialog with no console errors). Fixed by skipping the assignment when text already matches + a re-entry guard on the observer callback.
...
- **GPT-image-2 diagram evaluation** (22 Apr 2026) — OpenAI's new image model (released 21 Apr 2026) tested with 6 diverse diagrams. Quality significantly better than Gemini — accurate text rendering, scientific symbols (photosynthesis equation, binary addition), historically accurate imagery (Weimar wheelbarrow of banknotes, Jekyll/Hyde Victorian split-figure), geographic cross-sections (plate boundaries). Cost: ~£0.12/image at 1536×1024 high quality. Generation time: ~2 min/image (thinking mode). Strategy: **two-step pipeline** — Claude agent reads lesson, decides whether a diagram is genuinely additive, writes diagram type + content description → injected into fixed template (visual style, Mayer pedagogical principles, accuracy guardrails, per-unit accent colour) → GPT-image-2 generates. Full cost estimate for every GCSE article lesson across entire built-out platform: ~£400–700. Parked pending Tom's QA completion. Test outputs + prompts in `test-diagrams/`. Script: `scripts/_test_gpt_image_2.py`. See `memory/gpt_image_2_evaluation.md`.
- **New English Lit texts built** (20–22 Apr 2026) — AQA: Princess & The Hustler (7 lessons), My Name is Leon (7), Worlds & Lives Poetry (8). Eduqas: Poetry Anthology (8). A Taste of Honey and Leave Taking already existed and were verified. All 30 lessons have: content, hero images, unit card images, unit subtitles, lesson descriptions, narration (859 clips), podcasts, 171 related media links curated by 6 agent passes (all URLs verified). Three discontinued AQA texts removed: History Boys, Curious Incident, Never Let Me Go (last AQA exam was 2024). Total free tier English Lit now 775 lessons across 4 boards.
- **OCR Combined Science content verified clean** (22 Apr 2026) — 48 lessons checked for cross-board content contamination (the issue Edexcel had before its regen). Zero AQA markers, zero Edexcel markers, 29 lessons contain OCR-specific exam guidance ("OCR Gateway 6-mark questions" etc.). Content was correctly built from the OCR Gateway A spec.
- **Edexcel Combined Science podcasts regenerated** (confirmed 22 Apr 2026) — all 48 board-specific podcasts now on R2 at `science-edexcel/...`, audio verified different to AQA equivalents.
- **GCSE Subject Tracker expanded** (22 Apr 2026) — `data/gcse-subject-tracker.xlsx` now has 11 sheets: main tracker (193 specs, 26 built) + English Lit Texts (52 texts × 4 boards matrix with hero QA columns) + 9 per-subject hero QA sheets covering 358 article lessons (Combined Science, Separate Sciences, History, RE, CS, D&T, H&SC, H&C, Music Tech). Rebuilds from Supabase on every run. Script: `scripts/_gen_tracker.py`.
- **RE lesson titles fixed** (22 Apr 2026) — 12 lessons in christianity-practices, islam-beliefs, islam-practices had titles like "Lesson 1" instead of real topics. Now properly titled (e.g. "Salah & Sawm", "Tawhid, the Nature of Allah & Angels"). Content was always correct — just a metadata gap.
- **Homepage tagline hidden for returning free users** (22 Apr 2026) — "Free GCSE revision for every subject and exam board." hides once user has active free prefs. First-time visitors still see it.
- **Multi-board core subjects COMPLETE** (30 Mar 2026) — English Language (4 boards, 180 lessons), English Literature (4 boards, 775 lessons), Combined Science (3 boards, 144 lessons). All have: content, heroes, narration, related media, guide pages (exam technique + revision technique). All `pending_review` status.
...
- **Practice question format review (English Literature)**: Currently all practice questions on Eng Lit lessons are 30+4 mark full essay questions (matching AQA Modern Text format). Consider whether to: (a) keep as-is for exam authenticity, (b) add a varied mix (short quotation analysis, character tracking, extract analysis, comparison, plus full essay) to build skills progressively, or (c) reframe questions in proprietary wording to avoid reproducing exam board IP. Tension between exam preparation value vs copyright risk (see `GCSE Platform Copyright Risk Analysis.txt`). Needs legal advice before site-wide change. Applies to all boards, not just AQA.
