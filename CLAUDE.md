...
- **Foundation/Higher tier filtering COMPLETE** (3 Apr 2026) — 484 lessons across all tiered subjects. In-content `<div class="higher-only">` wrapping hides HT sections for Foundation students. Wizard tier picker → `studyvault-tiers` localStorage → `lesson-loader.js` adds `tier-foundation` body class → CSS hides `.higher-only`. Subjects: Maths (192 practice-format, F/H per lesson), Combined Science AQA/Edexcel/OCR (192 lessons, 53 wrapped), Separate Sciences AQA (22 lessons, 14 wrapped), French (26, 8 wrapped), German (26, 4 wrapped), Spanish (26, 9 wrapped).
