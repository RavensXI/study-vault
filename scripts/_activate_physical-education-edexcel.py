"""Phase 2 - Subject activation for Pearson Edexcel GCSE Physical Education (1PE0), free tier.

Reads scripts/_plan_physical-education-edexcel.json and creates:
  - 1 subjects row (status=live, school_id=null)
  - 2 units rows (sort_order 1..2)
  - 30 empty lesson shells (status=pending_review, content_html=null)

Idempotent guard: if a subject row with slug 'physical-education-edexcel' already
...
Those edits are handled separately in this activation run.
"""
...
plan = json.loads(
    (scripts / "_plan_physical-education-edexcel.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]   # physical-education-edexcel
...
# ============================================================ quote ticker HTML
# 6 quotes from plan, cycling across the 2 unit accents, doubled for seamless scroll.
...
subject_payload = {
    "name": plan["subject"]["name"],                  # Physical Education
    "slug": SUBJECT_SLUG,                              # physical-education-edexcel
    "exam_board": plan["subject"]["exam_board"],       # Edexcel
    "spec_code": plan["subject"]["spec_code"],         # 1PE0
    "school_id": SCHOOL_ID,                            # NULL
    "color": plan["subject"]["target_hero_colour"],    # #be123c
    "is_active": True,
    "status": "live",
    "sort_order": 0,
    "image_url": None,
