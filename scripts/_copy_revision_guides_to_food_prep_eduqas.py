...
dst_sid = sb.table("subjects").select("id").eq("slug", "food-preparation-and-nutrition-eduqas").is_("school_id", "null").execute().data[0]["id"]
...
for g in src_guides:
    ...
    # Food Prep is a Path A subject so write neutrally — "GCSE Food Preparation and Nutrition".
    html = re.sub(r"\bOCR Physical Education\b", "GCSE Food Preparation and Nutrition", html)
    html = re.sub(r"\bPhysical Education\b", "Food Preparation and Nutrition", html)
    # Update inter-guide nav links
    html = html.replace("/guide/physical-education-ocr/", "/guide/food-preparation-and-nutrition-eduqas/")
    title = re.sub(r"\bPhysical Education\b", "Food Preparation and Nutrition", g["title"] or "")
