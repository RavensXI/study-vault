...
from _designlab_unit_backdrops import ACCENT, MOTIFS, fallback_motifs, UNITS
from openai import OpenAI

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ASSETS = os.path.join(ROOT, "design-lab", "assets")
QUALITY = os.environ.get("DL_QUALITY", "medium")   # medium ≈ high for these soft backdrops, ~3x cheaper
...
def whatis(motifs):
    # tell the model what each sketched shape is meant to be, so an ambiguous blob isn't reinterpreted
    return (f" The drawing depicts these specific things: {motifs}. Render each shape as exactly what it is meant "
            f"to be — do NOT reinterpret any shape as a different object.")

def prompts(motifs, word, hexc):
    w = whatis(motifs)
    return {
      "blueprint": (f"Redraw this exact image as a precise technical BLUEPRINT: render every shape as clean confident "
                    ...
                    f"drafting style, on pale warm paper.{w} {KEEP}"),
      "refined":   (f"Redraw this exact image as a detailed PEN-AND-INK and soft-watercolour illustration: confident "
                    ...
                    f"naturalist's notebook plate, calm and not too dark.{w} {KEEP}"),
      "photo":     (f"Transform this exact sketch into a richly detailed PHOTOREALISTIC version: each object becomes a "
                    ...
                    f"fully resolved.{w} {KEEP}"),
    }

def motifs_for(subject, unit):
    m = MOTIFS.get(f"{subject}/{unit}")
    if m:
        return m
    u = next((u for u in UNITS.get(subject, {}).get("units", []) if u["slug"] == unit), None)
    return fallback_motifs(u) if u else unit.replace("-", " ")
...
def main():
    ...
    client = OpenAI()
    motifs = motifs_for(subject, unit)
    for stage, prompt in prompts(motifs, word, hexc).items():
        out = os.path.join(ASSETS, f"path-bg-u-{subject}-{unit}-{stage}.png")
        ok = False
        for model in ("gpt-image-2", "gpt-image-1"):
            try:
                print(f"[{stage}] {model} edit ({QUALITY})…", flush=True)
                with open(sketch, "rb") as img:
                    r = client.images.edit(model=model, image=img, prompt=prompt, size="1024x1536", quality=QUALITY)
