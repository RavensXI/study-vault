...
Jane Eyre ADJUDICATION pass (rulings on G3-G8).

The changed set is DERIVED, not assumed: for every lesson touched, narration
chunks are extracted from the pre-adjudication content in
_adjudication_backup.json and from the live post-adjudication content, then
compared id by id. Attribute-only edits (data-def, data-revision-tip) produce
no text delta and are skipped automatically.

Voice: odd lesson_number -> Ollie, even -> Ada (CLAUDE.md convention).
get_voice_for_lesson returns a TUPLE (voice_name, voice_label).

R2 key is DERIVED FROM THE EXISTING MANIFEST src, never hardcoded: this unit
mixes two prefixes (L2 uses english-literature-aqa/, the rest english-literature/).

Usage:
  python scripts/_retrofc/units/english-literature-aqa__jane-eyre/_adj_renarrate.py --dry-run
  python scripts/_retrofc/units/english-literature-aqa__jane-eyre/_adj_renarrate.py
"""
import argparse
import json
import os
...
LOGFILE = os.path.join(HERE, "_adj_renarrate.log")
...
def combined(row):
    return "".join((row.get(f) or "") for f in HTML_FIELDS)
...
def main():
    ...
    L("\n=== ADJUDICATION RE-NARRATION (changed narrated text only) ===")
    ...
    for entry in backup["lessons"]:
        ...
        if not any(f in before for f in HTML_FIELDS):
        ...
        old_row = {f: before.get(f, live.get(f)) for f in HTML_FIELDS}
    ...
    try:
        ...
        report = {}
    report.setdefault("adjudication", {})["renarration"] = {
