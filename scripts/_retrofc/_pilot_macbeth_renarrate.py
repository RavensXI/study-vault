...
Macbeth retro fact-check.

The changed set is DERIVED, not assumed: for every lesson touched, the script
extracts narration chunks from the pre-fix content in
_pilot_macbeth_backup.json and from the live post-fix content, then compares
them id by id. Attribute-only edits (data-def, data-revision-tip) produce no
text delta and are therefore skipped automatically.

Voice: odd lesson_number -> Ollie, even -> Ada (CLAUDE.md convention).
R2 key preserved exactly: english-literature/macbeth/narration_lesson-NN_nid.mp3

Usage:
  python scripts/_retrofc/_pilot_macbeth_renarrate.py --dry-run
  python scripts/_retrofc/_pilot_macbeth_renarrate.py
"""
import argparse
import json
import os
...
BACKUP = os.path.join(HERE, "_pilot_macbeth_backup.json")
LOGFILE = os.path.join(HERE, "_pilot_macbeth.log")
R2_PREFIX = "english-literature/macbeth"
...
def main():
    ...
    for entry in backup["lessons"]:
        ...
        # Attribute-only lessons still get diffed - cheap, and proves the point.
        if "content_html" not in before:
            L(f"  L{num:02d}: no content_html change -> skip")
            continue
    ...
    if args.dry_run:
        for p in plan:
            voice_name, voice_label = get_voice_for_lesson(p["num"])
    ...
    # ---- verify: HTTP 200 + fresh Last-Modified on every regenerated key ----
    ...
    for p in plan:
        for nid, _ in p["changed"]:
            ...
            url = f"{AUDIO_PUBLIC_URL}/{key}"
            # NOTE: the R2 public dev endpoint 403s on bare HEAD. Use a small
            # ranged GET instead - it also lets us confirm the MPEG frame sync.
            ...
            try:
                with urllib.request.urlopen(req, timeout=30) as r:
                    ...
                    ln = r.headers.get("Content-Range", r.headers.get("Content-Length", "?"))
                    good = r.status in (200, 206) and magic[:2] == b"\xff\xf3"
                    L(f"  {r.status}  L{p['num']:02d} {nid}  {ln}  Last-Modified: {lm}"
                      f"  mp3-sync={'OK' if magic[:2] == b'\xff\xf3' else magic[:3]!r}")
                    if good:
                        ok += 1
            except Exception as e:
                L(f"  FAIL L{p['num']:02d} {nid}: {e}")
    L(f"\n{ok}/{regenerated} clips verified HTTP 200.")
