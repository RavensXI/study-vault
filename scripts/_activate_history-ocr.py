"""Phase 2 — Subject activation for History OCR A (J410), free tier.

Reads scripts/_plan_history-ocr.json and creates:
  - 1 subjects row (status=live, school_id=null)
  - 12 units rows (sort_order 1..12)
  - 117 empty lesson shells (status=pending_review)

Idempotent guard: if a subject row with slug 'history-ocr' already exists
(school_id NULL) the script ABORTS without modifying anything
...
plan = json.loads(
    (scripts / "_plan_history-ocr.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]   # history-ocr
...
# ============================================================ quote ticker HTML
# 6 quotes from plan, cycling across the 12 unit accents, doubled for seamless scroll.
...
subject_payload = {
    "name": plan["subject"]["name"],           # History
    "slug": SUBJECT_SLUG,                       # history-ocr
    "exam_board": plan["subject"]["exam_board"],# OCR
    "spec_code": plan["subject"]["spec_code"],  # J410
    "school_id": SCHOOL_ID,                     # NULL
    "color": plan["subject"]["target_hero_colour"],  # #581c87
    "is_active": True,
    "status": "live",
    "sort_order": 0,
    "image_url": None,
