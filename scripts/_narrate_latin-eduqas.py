# -*- coding: utf-8 -*-
"""Generate Azure TTS narration for Latin (latin-eduqas) free-tier.

21 article-format lessons across 5 units. The three practice units carry
`practice_data` instead of `content_html` and are never narrated.
...
Latin is not in `SUBJECT_LANG_CODES`, so the extractor adds no SSML <lang>
switch: the British voice reads the Latin, which is how it is read aloud in a
UK classroom. Quotations in these lessons are deliberately short for that
reason.

R2 audio key: latin-eduqas/{unit_slug}/narration_lesson-{NN}_{nid}.mp3

Usage: python scripts/_narrate_latin-eduqas.py [--unit slug] [--lesson N] [--dry-run]
"""
import argparse
import os
import sys
...
def process_lesson(sb, r2_client, unit_slug, unit_id, lesson_number, dry_run=False):
    ...
    existing = lesson.get("narration_manifest")
    if isinstance(existing, list) and existing:
        print(f"  SKIP: narration_manifest already populated ({len(existing)} clips)")
        return 0, 0, 0.0

    combined_html = ((lesson.get("content_html") or "")
    ...
    for nid, text in chunks:
        r2_key = (f"{SUBJECT_SLUG}/{unit_slug}/"
                  f"narration_lesson-{lesson_number:02d}_{nid}.mp3")
        public_url = f"{AUDIO_PUBLIC_URL}/{r2_key}"
        total_chars += len(text)
        display = (text[:70] + "...") if len(text) > 70 else text
        print("    %s: %s" % (nid, display.encode("ascii", "replace").decode("ascii")))
    ...
    print(f"  Manifest updated ({len(manifest)} entries, {total_duration:.1f}s audio, "
          f"{elapsed:.1f}s)")
...
def main():
    ...
    print("Latin (Eduqas / WJEC) - Narration")
    print("=" * 55)

    sb = get_client()
    subjects = (sb.table("subjects").select("id, slug")
    ...
    units_result = (sb.table("units").select("id, slug")
    ...
    for unit_slug, unit_id, lesson_num in work_items:
        try:
            clips, chars, _ = process_lesson(sb, r2_client, unit_slug, unit_id,
                                             lesson_num, dry_run=args.dry_run)
            total_clips += clips
            total_chars += chars
