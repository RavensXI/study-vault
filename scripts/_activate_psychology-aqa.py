"""Phase 2 - Subject activation for Psychology (AQA 8182), free tier.

Reads scripts/_plan_psychology-aqa.json and creates the subject + 8 units
+ 32 empty lesson shells in Supabase. Updates subjects.settings with
...
plan = json.loads(
    (scripts / "_plan_psychology-aqa.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]  # psychology-aqa
...
# Quote ticker palette — cycled across the planned quotes. The .quote-ticker >
# .quote-ticker-track > .quote-item structure is required for the CSS scroll
# animation (bare spans will not animate). The full sequence is duplicated once
# for a seamless loop.
quote_palette = [
...
if len(quote_items) >= 2:
    quote_items = quote_items + quote_items
quote_ticker_html = (
...
for pu in plan["article_units"]:
    ...
    if slug in units_by_slug:
        ...
        print(f"  UPDATED: {slug:32s} ({pu['lesson_count']} lessons)")
    else:
        res = sb.table("units").insert({**payload_unit, "image_url": None}).execute()
        unit_rows[slug] = res.data[0]
        print(f"  INSERTED: {slug:32s} ({pu['lesson_count']} lessons)")
...
if total_in_db == expected:
    ...
    print(f"\n  slug -> unit_id map:")
    for pu in plan["article_units"]:
        print(f"    {pu['slug']:32s} {unit_rows[pu['slug']]['id']}")
else:
