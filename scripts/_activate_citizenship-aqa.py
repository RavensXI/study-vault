"""Phase 2 - Subject activation for Citizenship Studies (AQA 8100), free tier.

Reads scripts/_plan_citizenship-aqa.json and creates the subject + 2 units
+ 29 empty lesson shells in Supabase. Updates subjects.settings with
...
plan = json.loads(
    (scripts / "_plan_citizenship-aqa.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]  # citizenship-aqa
...
if existing:
    ...
    # Walk all lessons under this subject's units; if ANY have meaningful
    # content, abort and report.
    units_check = (
...
if SUBJECT_ID:
    # Preserve any keys we don't explicitly redefine.
