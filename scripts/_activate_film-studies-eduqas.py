"""Phase 2 - Subject activation for Film Studies (Eduqas C670QS), free tier.

Reads scripts/_plan_film-studies-eduqas.json and creates the subject + 5 units
+ 28 empty lesson shells in Supabase. Updates subjects.settings with
quote_ticker_html, question_type_names, practice_units (empty),
unit_image_positions, has_exam_guides=False.

DUAL-BOARD: This subject is the canonical row for BOTH Eduqas (English) and
WJEC (Welsh) Film Studies — the spec is identical. Wizard wiring routes both
board picks to this single `film-studies-eduqas` slug.
...
plan = json.loads(
    (scripts / "_plan_film-studies-eduqas.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]  # film-studies-eduqas
...
# Duplicate first 2 for seamless loop.
...
subject_payload = {
    ...
    "is_active": True,
    "status": "live",  # subject is live; lessons remain pending_review
    "sort_order": 1,
    "settings": new_settings,
    "image_url": None,  # filled by Unsplash hero pull in Phase 4
}
...
for pu in plan["article_units"]:
    ...
    if slug in units_by_slug:
        ...
        # image_url left null - the auto-sync trigger fills it from L1 hero.
...
for pu in plan["article_units"]:
    ...
    total_in_db += cnt
    unit_ids_report.append((pu["slug"], unit_id, cnt, pu["lesson_count"]))
    print(f"  {pu['slug']}: {cnt} lessons (planned {pu['lesson_count']}) [unit {unit_id[:8]}]")
