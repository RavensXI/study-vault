...
Workflow per lesson:
  1. Load hero_keywords from scripts/_content_engineering-aqa/lessons/{slug}.json
...
def main():
    ...
    LESSONS = (
        [("engineering-materials", n) for n in range(1, 6)] +
        [("manufacturing-processes", n) for n in range(1, 7)] +
        [("engineering-systems", n) for n in range(1, 8)] +
        [("testing-drawing-and-the-engineering-industry", n) for n in range(1, 5)]
    )
    ...
    for unit_slug, lesson_number in LESSONS:
        ...
        alt_text = f"{lesson_title} — Engineering"
