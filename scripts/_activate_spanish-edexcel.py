"""Phase 2 - Subject activation for Spanish (Edexcel 1SP1), free tier.

Reads scripts/_plan_spanish-edexcel.json and creates the subject + 2 units
+ 27 empty lesson shells in Supabase. Updates subjects.settings with
...
plan = json.loads(
    (scripts / "_plan_spanish-edexcel.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]  # spanish-edexcel
...
UNITS = plan.get("practice_units") or plan.get("article_units") or []

new_settings = {
    "has_exam_guides": False,
    "practice_units": [u["slug"] for u in UNITS],  # all units are practice-format
    "question_type_names": plan.get("question_type_names", []),
    "quote_ticker_html": quote_ticker_html,
    "unit_image_positions": {u["slug"]: "center" for u in UNITS},
}
...
expected = sum(u["lesson_count"] for u in UNITS)
