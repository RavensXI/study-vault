"""Phase 2 - Subject activation for Drama (AQA 8261), free tier.

Reads scripts/_plan_drama-aqa.json and creates the subject + 12 units
+ ~85 empty lesson shells in Supabase. Updates subjects.settings with
...
plan = json.loads(
    (scripts / "_plan_drama-aqa.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]  # drama-aqa
