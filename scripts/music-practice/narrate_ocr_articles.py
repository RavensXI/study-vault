# -*- coding: utf-8 -*-
"""Azure narration for the music-ocr article lessons (post fact-check).

The music-ocr article units (film L1-L3 carry copied manifests
and are skipped by the manifest-exists guard). Voice by house
convention (Ollie odd, Ada even).
R2: music-ocr/{unit_slug}/narration_lesson-{NN}_{nid}.mp3

Usage:
    python scripts/music-practice/narrate_ocr_articles.py --dry-run
    python scripts/music-practice/narrate_ocr_articles.py [--force]
"""
import argparse
import os
import sys
...
ARTICLE_UNITS = ["aos2-the-concerto-through-time",
                 "aos3-rhythms-of-the-world", "aos4-film-music",
...
def main():
    ...
    ap.add_argument("--only", help="unit-slug:lesson-number, e.g. aos1-forms-and-devices:2")
    args = ap.parse_args()
    only = None
    if args.only:
    ...
    sub = [x for x in sb.table("subjects").select("id,school_id").eq(
        "slug", "music-ocr").execute().data if not x["school_id"]][0]
    ...
    for uslug in ARTICLE_UNITS:
        ...
        for lesson in sb.table("lessons").select(
            ...
            for nid, text in chunks:
                key = "music-ocr/%s/narration_lesson-%02d_%s.mp3" % (uslug, num, nid)
