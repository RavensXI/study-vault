# -*- coding: utf-8 -*-
"""Azure narration for the music-edexcel article lessons (post fact-check).

The music-edexcel article units (film L1-L3 carry copied manifests
and are skipped by the manifest-exists guard). Voice by house
convention (Ollie odd, Ada even).
R2: music-edexcel/{unit_slug}/narration_lesson-{NN}_{nid}.mp3

Usage:
    python scripts/music-practice/narrate_edexcel_articles.py --dry-run
    python scripts/music-practice/narrate_edexcel_articles.py [--force]
"""
import argparse
import os
import sys
...
ARTICLE_UNITS = ["aos1-instrumental-music", "aos2-vocal-music",
...
def main():
    ...
    sub = [x for x in sb.table("subjects").select("id,school_id").eq(
        "slug", "music-edexcel").execute().data if not x["school_id"]][0]
    ...
    for uslug in ARTICLE_UNITS:
        ...
        for lesson in sb.table("lessons").select(
            ...
            for nid, text in chunks:
                key = "music-edexcel/%s/narration_lesson-%02d_%s.mp3" % (uslug, num, nid)
