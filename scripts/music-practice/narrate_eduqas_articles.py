# -*- coding: utf-8 -*-
"""Azure narration for the music-eduqas article lessons (post fact-check).

Fourteen lessons across the four AoS article units. Voice by house
convention (Ollie odd, Ada even).
R2: music-eduqas/{unit_slug}/narration_lesson-{NN}_{nid}.mp3

Usage:
    python scripts/music-practice/narrate_eduqas_articles.py --dry-run
    python scripts/music-practice/narrate_eduqas_articles.py [--force]
"""
import argparse
import os
import sys
...
ARTICLE_UNITS = ["aos1-forms-and-devices", "aos2-music-for-ensemble",
...
def main():
    ...
    sub = [x for x in sb.table("subjects").select("id,school_id").eq(
        "slug", "music-eduqas").execute().data if not x["school_id"]][0]
    ...
    for uslug in ARTICLE_UNITS:
        ...
        for lesson in sb.table("lessons").select(
            ...
            for nid, text in chunks:
                key = "music-eduqas/%s/narration_lesson-%02d_%s.mp3" % (uslug, num, nid)
