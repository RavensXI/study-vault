"""Phase 2 - Subject activation for Sociology (Eduqas/WJEC C200QS / 3200QS), free tier.

Reads scripts/_plan_sociology-eduqas.json and creates the subject + 5 units
+ 33 empty lesson shells in Supabase. Updates subjects.settings with
quote_ticker_html, question_type_names, practice_units (empty),
unit_image_positions, has_exam_guides=False.

Path A twin build: this single Supabase row (slug `sociology-eduqas`) serves
both Eduqas (C200QS, England) and WJEC (3200QS, Wales). The two specifications
are byte-identical. WJEC alias is added to slugMap and boardConfig in
index.html post-build (separate later step — NOT this script).
...
plan = json.loads(
    (scripts / "_plan_sociology-eduqas.json").read_text(encoding="utf-8")
)
sb = get_client()

SUBJECT_SLUG = plan["subject"]["slug"]  # sociology-eduqas
...
# Plan has spec_codes (array) — join into single column value.
spec_codes = plan["subject"].get("spec_codes")
if spec_codes:
    spec_code_value = " / ".join(spec_codes)
else:
    spec_code_value = plan["subject"].get("spec_code", "")
