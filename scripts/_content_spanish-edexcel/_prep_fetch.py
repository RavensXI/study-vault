"""One-shot prep script for Edexcel Spanish (1SP1) Phase 3 scaffolding.

- Fetches the Edexcel subject + 27 lesson IDs from Supabase (subjects.slug = 'spanish-edexcel').
- Pulls the AQA Spanish source lessons' practice_data for every plan lesson with
  transfer_score in (high, medium). Writes them to _source/aqa_<unit>_lesson_<N>.json.
- Pulls a complete AQA Spanish lesson with practice_data as the canonical
...
Run from repo root: python scripts/_content_spanish-edexcel/_prep_fetch.py
"""
import json
import os
import sys
...
PLAN_PATH = SCRIPTS_DIR / "_plan_spanish-edexcel.json"
    ...
    for L in unit["lessons"]:
        ...
        out_path.write_text(
            json.dumps(
                {
                    "_source_meta": {
                        "source_subject_slug": "spanish-aqa",
...
# ============================================================ Reference lesson
print("\n=== Pulling canonical reference lesson (AQA Spanish people-and-lifestyle L01) ===\n")
    ...
    json.dumps(
        {
            "_source_meta": {
                ...
                "purpose": "Canonical structural reference for Edexcel Spanish Phase 3 content agents. Match SHAPE only — adapt vocab and Edexcel theme framing.",
            },
...
SHARED = {
    "spec_slice_path": "scripts/_content_spanish-edexcel/_spec_spanish-edexcel.txt",
    "reference_lesson_path": "scripts/_content_spanish-edexcel/_reference_lesson.json",
    "schema_path": "scripts/language-practice/PRACTICE_DATA_SCHEMA.md",
    "agent_prompt_path": "scripts/_content_spanish-edexcel/_AGENT_PROMPT.md",
    ...
    "output_dir": "scripts/_content_spanish-edexcel/lessons",
}
...
for unit in plan["practice_units"]:
    ...
    for L in unit["lessons"]:
        ...
        if ct["transfer_score"] in ("high", "medium"):
            ...
            if candidate.exists():
                source_aqa_file = f"scripts/_content_spanish-edexcel/_source/aqa_{src_unit}_lesson_{src_num}.json"
