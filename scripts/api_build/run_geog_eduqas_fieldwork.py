# -*- coding: utf-8 -*-
"""Append two cohort-specific fieldwork lessons to geography-eduqas /
fieldwork-enquiry (13 Sep 2026).

WJEC names one methodological approach (Table A) and one conceptual
framework (Table B) per exam year, and Component 3 Parts A and B are set on
that pair. The unit's three existing lessons teach the six-stage enquiry
cycle every question draws on; these two teach only the named pair for one
cohort and are keyed to that year in subjects.settings.exam_year_lessons so
a student sees just their own:

    L4  2027  Change over time + Sphere of influence
    L5  2028  Use of transects + Place

Reuses driver.py's Batch-API stages (submit / poll / fix / factcheck /
applyfixes) and differs in three places:

  * prep adds a per-lesson COHORT block (plan lesson["route"]) so each
    request knows its year's pair and that every other pair is out of scope.
  * insert POSTS new rows (the driver's insert patches rows that already
    exist), status pending_review, tier both.
  * three extra stages finish the rows: heroes (vision-gated photograph via
    lib.hero_pipeline), narrate (scripts/_narrate_single_lesson.py), gate
    (writes the exam_year_lessons keys).

    python run_geog_eduqas_fieldwork.py --config <cfg.json> prep
    ... submit | poll | fix | pollfix | factcheck | pollfactcheck
    ... applyfixes | pollapplyfixes | insert | heroes | narrate | gate | costs
"""
import argparse
...
def stage_narrate(cfg):
    ...
    rows = drv.supa(cfg, "GET", "/rest/v1/lessons?unit_id=eq.%s&select=id,lesson_number"
                    "&lesson_number=in.(%s)" % (unit_id, ",".join(str(n) for n in numbers)))
    for r in rows:
