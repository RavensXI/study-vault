...
import jev
from jev import ask_many, answers, Score, Choice
jev.BUDGET_USD = 12.0        # topped up 19 Sep; the fleet pass is about $2

CAP = 14; KIND_MAX = 7; DUP_P = 0.6
...
def choose(cards, sc):
    for c in cards:
        s = sc.get(c["id"], {}); c["central"] = s.get("central", 1.0); c["dup"] = s.get("dup"); c["dup_p"] = s.get("dup_p", 0.0)
...
def run(slug, school, apply):
    ...
    jobs = [j for j in jobs if len(j["cards"]) > CAP]
    ...
    for j, r, err in res:
        ...
        record.append({"id": j["lesson"]["id"], "unit": j["lesson"]["units"]["slug"], "n": j["lesson"]["lesson_number"], "title": j["lesson"]["title"], "deck": len(j["cards"]), "after": len(keep),
                       ...
                       "drop": [{"id": c["id"], "kind": c["kind"], "src": c["src"], "front": c["front"], "answer": c["answer"], "central": round(c["central"], 2), "why": c["why"]} for c in drop]})
