...
// A subject whose own landing page redirects elsewhere (browse-loader.js): list its units only.
const NO_LANDING = new Set(['separate-sciences']);
...
${u.lessons.map(l => `      <li><a href="${esc(l.path)}">${esc(l.title)}</a></li>`).join('\n')}
  ...
  return page({
    ...
    <p class="lede">${count} free revision lessons in ${units.length} units.${NO_LANDING.has(subject.slug) ? '' : ` <a href="/browse/${esc(subject.slug)}">Open the course</a> or go straight to a lesson.`}</p>
${body}
  ...
  try {
    ...
    if (kind === 'sitemap') {
      ...
      if (name === 'pages') {
        ...
        entries = NO_LANDING.has(name) ? [] : [{ path: `/browse/${name}`, lastmod: newest(units.flatMap(u => u.lessons.map(l => l.lastmod))) }];
        units.forEach(u => {
